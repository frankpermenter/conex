 ...
 * \endlink mechanism.
 *
 * \sa MatrixBase::rldlt(), SelfAdjointView::rldlt(), class LLT
 */
...
class RLDLT {
 public:
  ...
  /** \brief Constructs a RLDLT factorization from a given matrix
   *
   ...
   *
   * \sa MatrixBase::rldlt(), SelfAdjointView::rldlt()
   */
  template <typename Rhs>
  inline const Solve<RLDLT, Rhs> solve(const MatrixBase<Rhs>& b) const {
    eigen_assert(m_isInitialized && "RLDLT is not initialized.");
    eigen_assert(m_matrix.rows() == b.rows() &&
                 "RLDLT::solve(): invalid number of rows of the right hand "
                 "side matrix b");
    return Solve<RLDLT, Rhs>(*this, b.derived());
  }
  ...
  template <typename InputType>
  RLDLT& compute(const EigenBase<InputType>& matrix);

  /** \returns an estimate of the reciprocal condition number of the matrix of
   *  which \c *this is the RLDLT decomposition.
   */
  ...
  template <typename Derived>
  RLDLT& rankUpdate(const MatrixBase<Derived>& w, const RealScalar& alpha = 1);

  /** \returns the internal RLDLT decomposition matrix
   *
   * TODO: document the storage layout
   */
  inline const MatrixType& matrixRLDLT() const {
    eigen_assert(m_isInitialized && "RLDLT is not initialized.");
    return m_matrix;
  }
   ...
   */
  const RLDLT& adjoint() const { return *this; };
...
struct rldlt_inplace<Lower> {
  ...
  static bool unblocked(MatrixType& mat, Workspace& temp, SignMatrix& sign) {
    ...
    for (Index k = 0; k < size; ++k) {
      ...
      // RLDLT is not rank-revealing we should only make sure that we do not
      ...
      if (k == 0 && !pivot_is_valid) {  // The entire diagonal is zero, there is
                                        // nothing more to do
  ...
  static bool update(MatrixType& mat, const TranspositionType& transpositions,
    ...
    return rldlt_inplace<Lower>::updateInPlace(mat, tmp, sigma);
  }
};

template <>
struct rldlt_inplace<Upper> {
  template <typename MatrixType, typename TranspositionType, typename Workspace>
  static EIGEN_STRONG_INLINE bool unblocked(MatrixType& mat,
    ...
    return rldlt_inplace<Lower>::unblocked(matt, transpositions, temp, sign);
  }
  ...
  static EIGEN_STRONG_INLINE bool update(
    ...
    Transpose<MatrixType> matt(mat);
    return rldlt_inplace<Lower>::update(matt, transpositions, tmp,
                                        w.conjugate(), sigma);
  }
};

template <typename MatrixType>
struct RLDLT_Traits<MatrixType, Lower> {
...
template <typename MatrixType>
struct RLDLT_Traits<MatrixType, Upper> {
  typedef const TriangularView<const typename MatrixType::AdjointReturnType,
                               UnitLower>
      MatrixL;
...
/** Compute / recompute the RLDLT decomposition A = L D L^* = U^* D U of \a
 * matrix
 */
template <typename MatrixType, int _UpLo>
template <typename InputType>
RLDLT<MatrixType, _UpLo>& RLDLT<MatrixType, _UpLo>::compute(
...
/** Update the RLDLT decomposition:  given A = L D L^T, efficiently compute the
...
void RLDLT<_MatrixType, _UpLo>::_solve_impl(const RhsType& rhs,
  ...
  // NumTraits<RealScalar>::highest()); However, RLDLT is not rank revealing,
...
/** \internal use x = rldlt_object.solve(x);
 *
 ...
 * needed anymore.
 *
 * \sa RLDLT::solve(), MatrixBase::rldlt()
 */
template <typename MatrixType, int _UpLo>
template <typename Derived>
bool RLDLT<MatrixType, _UpLo>::solveInPlace(MatrixBase<Derived>& bAndX) const {
  eigen_assert(m_isInitialized && "RLDLT is not initialized.");
...
template <typename MatrixType, int _UpLo>
MatrixType RLDLT<MatrixType, _UpLo>::reconstructedMatrix() const {
  eigen_assert(m_isInitialized && "RLDLT is not initialized.");
