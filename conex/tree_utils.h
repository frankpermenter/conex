...
std::vector<int> GetChildren(const RootedTree& d, int parent);

std::vector<int> GetRootNodes(const RootedTree& d);

vector<int> GetUnvisitedLeafNode(const std::vector<int>& num_children,
                                 const std::vector<int>& visited);

std::vector<std::vector<int>> PartitionIntoPaths(const RootedTree& tree);
