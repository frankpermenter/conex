...
void BasicHSDSolverTestHelper(const Eigen::MatrixXd& A,
  ...
  if (constraints_per_block == -1) {
    ...
    if ((A.cols() / constraints_per_block) * constraints_per_block !=
        A.cols()) {
      EXPECT_TRUE(false);
  ...
  for (int i = 0; i < c.rows() / constraints_per_block; i++) {
    prog.AddConstraint(LinearConstraint(
        A.middleCols(offset, constraints_per_block).transpose(),
        c.segment(offset, constraints_per_block)));
  ...
  Eigen::MatrixXd Asocp(3, 3);
  Asocp.setIdentity();
  Eigen::MatrixXd csocp(3, 1);
  csocp.setZero();
  csocp(0) = 10;
...
void DirectionTestsHelper(const Eigen::MatrixXd& A, const Eigen::VectorXd& bin,
  ...
  Eigen::VectorXd b(prog.constraint_manager().SizeOfKKTSystem());
  b.setZero();
  b.head(m) << bin;

  auto& sys = prog.kkt_system_residual();
  ...
  for (int i = 0; i < 15; i++) {
    ...
    prog.kkt_solver()->Assemble();
    prog.kkt_solver()->Factor();
    AssembleSchurComplementResiduals(prog.constraint_manager(), &sys);
    EXPECT_NEAR((sys.AW.topRows(m) - A * w).norm() / (A * w).norm(), 0, eps);
    EXPECT_NEAR((sys.AQc.topRows(m) - A * Qw * c).norm() / (A * Qw * c).norm(),
                0, eps);
    EXPECT_NEAR((sys.AQe.topRows(m) - A * Qw * e).norm() / (A * Qw * e).norm(),
                0, eps);
    EXPECT_NEAR((sys.Ae.topRows(m) - A * e).norm() / (A * e).norm(), 0, eps);

    EXPECT_NEAR(sys.inner_product_of_c_and_e, c.transpose() * e,
                eps * (c.transpose() * e).norm());
    EXPECT_NEAR(sys.inner_product_of_w_and_c, c.transpose() * w,
                eps * (c.transpose() * w).norm());
    EXPECT_NEAR(sys.inner_product_of_c_and_Qc, c.transpose() * Qw * c,
                (c.transpose() * Qw * c).norm() * eps);
    EXPECT_NEAR(sys.inner_product_of_c_and_Qe, c.dot(Qw * e),
                std::abs(c.dot(Qw * e)) * eps);
    ...
    auto sol = SolveEmbedding(sys, prog.kkt_solver(), b, wt, sqrtmu);
    VectorXd y = sol.sol2;
    ...
    PrepareStep(&prog.constraint_manager(), options, ym, &info);
    options.step_size = 2.0 / (info.norminfd * info.norminfd);
    if (options.step_size > 1) {
      options.step_size = 1;
    }
    TakeStep(&prog.constraint_manager().cone_inequalities(), options);
    ...
    VectorXd errX;
    if (B.rows() > 0) {
      errX = A * (w + Qwsqrt * d) + B.transpose() * y.tail(B.rows()) -
             (wt * (1 + dt) * b.head(m) + sqrtmu * (A * e - b.head(m)));
    } else {
      errX = A * (w + Qwsqrt * d) -
             (wt * (1 + dt) * b.head(m) + sqrtmu * (A * e - b.head(m)));
    }
    ...
    std::cout << "\n tau: " << sqrtmu * wt << "  d:" << dinf
              << "  dinf_step:" << info.norminfd << "  sqrtmu:" << sqrtmu
              << " , " << errX.norm() << " , " << errS.norm() << ","
              << errG1 - errG2;
    ...
    if (dinf < 1) {
      if (sqrtmu * sqrtmu < 1e-10) {
        ...
        if (B.cols() > 0) {
          EXPECT_NEAR((A * x + B.transpose() * y.tail(B.rows()) * sqrtmu / tau -
                       b.head(m))
                          .norm(),
                      0, eps);
        } else {
