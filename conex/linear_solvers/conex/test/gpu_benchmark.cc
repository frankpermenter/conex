// Benchmark: GpuTreeSolver vs cuSOLVER sparse Cholesky on block-arrow matrices.
//
// A block-arrow matrix has the structure:
//
//   [D1          S1^T]
//   [   D2       S2^T]
//   [      ...   ... ]
...
//   [S1 S2 ... Sk  C ]
//
// Each diagonal block Di is block_size x block_size, the coupling block C
// is sep_size x sep_size, and Si is sep_size x block_size.  This structure
// produces a clique tree with k leaf supernodes (one per diagonal block)
// sharing a common separator (the coupling variables).  The supernodal
// tree solver should be efficient here since the dense blocks are large
// relative to the sparse fill-in.
//
// Timing covers only numeric factorization + solve (data already on device).
...
// Build a block-arrow least-squares problem.
//   A has num_blocks groups of rows_per_block rows each.
//   Group i touches columns [i*block_size .. (i+1)*block_size-1] (diagonal)
//   plus the last sep_size columns (coupling).
//   n = num_blocks * block_size + sep_size
BenchProblem MakeBlockArrow(int num_blocks, int block_size, int sep_size,
  ...
  int cols_per_row = block_size + sep_size;
...
// ============================================================================
// Assembly helper (shared by tree solver benchmark)
// ============================================================================

struct AssembledBlocks {
  std::vector<Eigen::MatrixXd> blocks;
  std::vector<std::vector<int>> clique_var_list;
};
...
AssembledBlocks AssembleBlocks(const BenchProblem& prob) {
  ...
  AssembledBlocks out;
  out.clique_var_list.resize(num_cliques);
  std::vector<std::unordered_map<int, int>> var_to_local(num_cliques);
  for (int ci = 0; ci < num_cliques; ++ci) {
    auto& vars = out.clique_var_list[ci];
  ...
  for (int ci = 0; ci < num_cliques; ++ci)
    out.blocks[ci] =
        Eigen::MatrixXd::Zero(out.clique_var_list[ci].size(),
                               out.clique_var_list[ci].size());
  ...
  for (int r = 0; r < A_row.rows(); ++r) {
    ...
    for (const auto& ei : row_entries) {
      ...
      for (const auto& ej : row_entries) {
        ...
        out.blocks[owner](it_i->second, it_j->second) += ei.second * ej.second;
      }
    }
  }
...
TimingResult BenchTreeSolver(const BenchProblem& prob, int warmup, int trials) {
  ...
  // Pre-build the solver and load data (not timed).
  GpuTreeSolver gpu;
  gpu.Finalize(ct);
  for (int ci = 0; ci < num_cliques; ++ci)
  ...
  for (int t = 0; t < warmup + trials; ++t) {
    // Re-upload data each trial (assembly overwrites factored data).
    gpu.SetSupernodeData(0, assembled.blocks[0]);  // trigger re-assembly
    for (int ci = 0; ci < num_cliques; ++ci)
      gpu.SetSupernodeData(ci, assembled.blocks[ci]);
...
TimingResult BenchCuSolverSp(const BenchProblem& prob, int warmup,
  ...
  // Build AtA lower triangle in CSR.
  ...
  for (int i = 0; i < n; ++i) {
    for (Eigen::SparseMatrix<double, Eigen::RowMajor>::InnerIterator it(
      ...
      if (col <= i) {
        ...
        csr_val.push_back(it.value());
      }
    }
    csr_row_ptr[i + 1] = static_cast<int>(csr_col_ind.size());
  }
  int nnz = static_cast<int>(csr_val.size());

  // Upload to device (not timed).
  ...
  CheckCuda(cudaMemcpy(d_row, csr_row_ptr.data(), (n + 1) * sizeof(int),
                        cudaMemcpyHostToDevice), "cp row");
  CheckCuda(cudaMemcpy(d_col, csr_col_ind.data(), nnz * sizeof(int),
                        cudaMemcpyHostToDevice), "cp col");
  CheckCuda(cudaMemcpy(d_val, csr_val.data(), nnz * sizeof(double),
                        cudaMemcpyHostToDevice), "cp val");
  CheckCuda(cudaMemcpy(d_b, prob.rhs.data(), n * sizeof(double),
                        cudaMemcpyHostToDevice), "cp b");
...
int main() {
  ...
  printf("Block-Arrow Benchmark: GPU Tree Solver vs cuSOLVER Sparse Cholesky vs CPU\n");
  printf("==========================================================================\n\n");

  // Block-arrow configs: (num_blocks, block_size, sep_size, rows_per_block)
  struct Config {
    int num_blocks;
    int block_size;
    int sep_size;
  ...
  std::vector<Config> configs = {
      // Small problems — many small blocks, small separator.
      {10, 10, 5, 15},
      {20, 10, 5, 15},
      {20, 20, 10, 30},
      // Medium — larger blocks.
      {10, 50, 10, 60},
      {20, 50, 10, 60},
      {10, 100, 10, 120},
      // Large — big dense blocks (tree solver should shine here).
      {10, 100, 20, 120},
      {20, 100, 20, 120},
      {10, 200, 20, 240},
      {5, 400, 20, 480},
  ...
  printf("%-5s %-5s %-5s %-5s %-6s | %-9s %-9s %-9s | %-9s %-9s %-9s | %-9s %-9s %-9s | %s\n",
         "blks", "bsz", "sep", "rpb", "n",
         "gpu_fac", "gpu_slv", "gpu_tot",
         "cusp_fac", "cusp_slv", "cusp_tot",
         "cpu_fac", "cpu_slv", "cpu_tot",
         "gpu/cusp");
  ...
  for (const auto& cfg : configs) {
    ...
    double gpu_tot = gpu.factor_ms + gpu.solve_ms;
    double cusp_tot = cusp.factor_ms + cusp.solve_ms;
    double cpu_tot = cpu.factor_ms + cpu.solve_ms;

    printf("%-5d %-5d %-5d %-5d %-6d | %8.2f  %8.2f  %8.2f  | %8.2f  %8.2f  %8.2f  | %8.2f  %8.2f  %8.2f  | %.2fx\n",
           cfg.num_blocks, cfg.block_size, cfg.sep_size,
           cfg.rows_per_block, n,
           gpu.factor_ms, gpu.solve_ms, gpu_tot,
           cusp.factor_ms, cusp.solve_ms, cusp_tot,
           cpu.factor_ms, cpu.solve_ms, cpu_tot,
           cusp_tot / gpu_tot);

    if (gpu.rel_err > 1e-6 || cusp.rel_err > 1e-6 || cpu.rel_err > 1e-6) {
      printf("  WARNING: rel_err — gpu: %.2e, cusp: %.2e, cpu: %.2e\n",
             gpu.rel_err, cusp.rel_err, cpu.rel_err);
    }
  }
