...
// Test right-looking factorization (left_looking = false).
// Right-looking scatters the Schur complement to the parent immediately
// after factoring each node, exercising DoScatterSeparatorSubmatrix and
// ReceiveColumnUpdate.
GTEST_TEST(SparseLeastSquares, RightLookingFactorization) {
  srand(42);
  int num_vars = 50;
  int bandwidth = 5;
  int rows_per_group = 10;
  ...
  // Right-looking: left_looking = false.
  Eigen::VectorXd b_zero = Eigen::VectorXd::Zero(A.rows());
  auto slc = std::make_unique<SparseLinearConstraint>(A, b_zero);
  std::set<int> var_set;
  for (const auto& support : slc->row_supports()) {
  ...
  SolverConfiguration config;
  config.tree.left_looking = false;
  auto tree_solver = MakeTreeSolver(&cm, config);
  ...
  // Compare against left-looking (default) to verify identical results.
  auto slc2 = std::make_unique<SparseLinearConstraint>(A, b_zero);
  ConstraintManager cm2(num_vars);
  auto assembler2 = std::make_unique<SparseLinearConstraintAssembler>(
      std::move(slc2), all_vars);
  ...
  SolverConfiguration config2;
  config2.tree.left_looking = true;
  auto tree_solver2 = MakeTreeSolver(&cm2, config2);
  ...
  VectorXd sol2 = tree_solver2->Solve(rhs);
  EXPECT_NEAR((sol - sol2).norm(), 0, 1e-10 * x_true.norm());
}

// Test right-looking with separate Assemble() + Factor() steps.
GTEST_TEST(SparseLeastSquares, RightLookingAssembleThenFactor) {
  srand(88);
  int num_blocks = 4;
  int rows_per_block = 6;
  int cols_per_block = 3;
...
// Uses aggressive clique merging to create tree nodes with many children,
// which triggers the threaded accumulate path in ApplyLeftLookingChildUpdates
// (requires num_threads > 1, children > 1, and update entries > 16384).
GTEST_TEST(SparseLeastSquares, MultithreadedLeftLooking) {
  srand(77);
  // Star-like structure: many small cliques all sharing a common set of
  // variables.  After merging, the root has many children with large blocks.
  int hub_size = 20;
  int spoke_size = 10;
  int num_spokes = 30;
  int rows_per_spoke = 15;
  ...
  for (int s = 0; s < num_spokes; s++) {
    ...
    for (int r = 0; r < rows_per_spoke; r++) {
      ...
      // Each row touches some hub variables and all spoke variables.
