...
std::vector<SupernodalAssemblerBase*> SparsePSDAssembler::Decompose(
...
SparsePSDLeastSquaresResult SparsePSDLeastSquaresImpl(
    const QType& Q,
    const Eigen::SparseMatrix<double>& A,
    const Eigen::VectorXd& rhs) {
  using clock = std::chrono::high_resolution_clock;
  SparsePSDLeastSquaresResult result;
  ...
  for (int i = 0; i < num_vars; ++i) var_set.insert(i);
  ...
  auto q_assembler = std::make_unique<SparsePSDAssembler>(Q, all_vars);
...
SparsePSDLeastSquaresResult SparsePSDLeastSquares(
    ...
    const Eigen::SparseMatrix<double>& A,
    const Eigen::VectorXd& rhs) {
  return SparsePSDLeastSquaresImpl(Q, A, rhs);
}
