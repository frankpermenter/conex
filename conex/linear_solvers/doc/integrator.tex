...
Viewing $v_1$ as a function of $h$ and differentiating both sides of \eqref{eq:vel_update} with respect to $h$:
\begin{equation}
    H(z_1)z_1' = \nabla^3 \phi(z_1)[z_1', 2hv_1 - (z_1 - z_0)] + H(z_1)(2v_1 + 2hv_1' - z_1')
\end{equation}
Evaluating at $h=0$ and using $z'(0) = v_0$ (as just proven), 
\begin{equation}
    H_0 v_0 = 0 + H_0(2v_1(0) - v_0) 
\end{equation}
This yields $2H_0 v_1(0) = 2H_0 v_0$, thus $b_0 = v_1(0) = v_0$.

To find $b_1 = v_1'(0)$, we expand \eqref{eq:vel_update} to order $h^2$. Substituting $b_0 = v_0$ and $H_0 a_2 = -\frac{1}{2}\nabla^3 \phi[v_0, v_0]$ into the expansion:
\begin{itemize}
    \item LHS: $\Delta \lambda = h H_0 v_0 + \frac{h^2}{4}\nabla^3 \phi[v_0, v_0] + \mathcal{O}(h^3)$
    \item RHS: $h H_0 v_0 + h^2 (2H_0 b_1 - \frac{1}{2}H_0 a_2 + \nabla^3 \phi[v_0, v_0]) + \mathcal{O}(h^3)$
\end{itemize}
Equating the $h^2$ coefficients:
\begin{equation}
    \frac{1}{4}\nabla^3 \phi[v_0, v_0] = 2H_0 b_1 + \frac{1}{4}\nabla^3 \phi[v_0, v_0] + \nabla^3 \phi[v_0, v_0]
\end{equation}
\begin{equation}
    2H_0 b_1 = -\nabla^3 \phi[v_0, v_0] \implies b_1 = -\frac{1}{2}H_0^{-1}\nabla^3 \phi[v_0, v_0, \cdot]
\end{equation}
This matches the continuous geodesic acceleration $\gamma''(0)$. Thus, $v_1(h) = \gamma'(0) + h \gamma''(0) + \mathcal{O}(h^2)$, which confirms $v_1(h) = \gamma'(h) + \mathcal{O}(h^3)$ due to the symmetry of the integrator.
