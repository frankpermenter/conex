...
\section{Beyond self-concordance: $M$-concordant barriers}
\label{sec:m-concordance}

Self-concordance ($M = 2$) is the standard regularity condition for
IPM barriers.  We show that higher-order integrators relax this
requirement, enabling a wider family of barriers.

\subsection{$M$-concordance}

\begin{definition}
A convex function $F$ is \emph{$M$-concordant} if
\begin{equation}\label{eq:m-concordance}
  |D^3 F(s)[v, v, v]| \leq M \|v\|_H^3
\end{equation}
for all $s \in \operatorname{int}(K)$ and $v \in \R^n$.  Standard
self-concordance is $M = 2$.
\end{definition}

Larger $M$ means the Hessian varies faster---the barrier is ``less
regular.''  An $M$-concordant barrier may be cheaper to evaluate,
simpler to construct, or available for cones where no $M = 2$
barrier is known.

\subsection{Step-size bounds under $M$-concordance}

From the error bounds in the previous section, the maximum step
$\delta = h\|v\|_H$ for staying in the Dikin ellipsoid depends on
both $M$ and the integrator order.

\paragraph{Tangent line.}
Error $\leq \frac{M}{4}\delta^2$.  Interior if $\delta < 2/\sqrt{M}$.

\paragraph{Bregman midpoint.}
The midpoint quadrature error is controlled by the variation of
$\dot\lambda = -Hv$, which involves $D^3 F$.  The bound:
\[
  \text{error} \leq C_2 \frac{M}{M_0}\delta^3,
\]
where $M_0 = 2$ and $C_2$ is a universal constant.
Interior if $\delta < (C_2 M / M_0)^{-1/3}$.

\paragraph{Yoshida-$2p$.}
The Yoshida composition cancels error terms up to order $2p$:
\[
  \text{error} \leq C_p \left(\frac{M}{M_0}\right)^{p} \delta^{2p+1}.
\]
Interior if $\delta < (C_p (M/M_0)^p)^{-1/(2p+1)}$.

\subsection{The trade-off: integrator order vs.\ barrier regularity}

For fixed accuracy (error $= \varepsilon$), the required step size is:
\[
  \delta_{\max} \propto \left(\frac{\varepsilon}{M^p}\right)^{1/(2p+1)}.
\]
As $M$ increases (worse barrier), $\delta_{\max}$ shrinks.  But the
...
\cmidrule(l){3-6}
\textbf{Method} & \textbf{Order} & $M\!=\!2$ & $M\!=\!10^2$ & $M\!=\!10^4$ & $M\!=\!10^6$ \\
\midrule
...
At $M = 10^6$ (far from self-concordant), the tangent line requires
$\delta < 0.0014$ (thousands of tiny steps), while Yoshida-6 permits
$\delta < 0.14$ (a few large steps).  The higher-order method
...
The Yoshida hierarchy converts integrator sophistication into barrier
generality.  Each level relaxes the concordance requirement:
\[
  M^{-1/2} \;\to\; M^{-1/3} \;\to\; M^{-1/5} \;\to\; M^{-1/7} \;\to\; \cdots
\]
This opens the door to barriers that classical (tangent-line) IPMs
cannot use, at the cost of a few extra gradient and Hessian evaluations
per step.
