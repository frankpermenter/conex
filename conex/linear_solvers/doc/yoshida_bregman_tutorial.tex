...
At the midpoint ($c = 1/2$), the Bregman velocity is
$v = (s_1 - s_{1/2})/(h/2)$, which is second-order accurate because
the midpoint symmetry cancels the first-order error.

At a general GL node ($c_i \neq 1/2$), the analogous chord velocity
$v_i = -H(s_i)^{-1}(\lambda_i - \lambda_0)/c_i$ is only \emph{first-order}
accurate.  The GL consistency conditions couple $v_1$ and $v_2$ through
the Butcher matrix $a_{ij}$, but without the geodesic acceleration
(which requires $\partial^3 F$), the coupled system is underdetermined.
...
\paragraph{Summary.}
The midpoint is special: it is the only evaluation point where the
Legendre transform provides both position \emph{and} velocity
information to second order.  Yoshida's genius is to achieve fourth
order by composing three midpoint evaluations with the right weights,
rather than attempting to evaluate at non-midpoint nodes (which would
require the ODE acceleration).
