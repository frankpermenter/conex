...
\paragraph{Why is this second-order?}
The primal step approximates the geodesic to first order (it's a
tangent line).  The dual step \emph{also} approximates the geodesic to
first order (a tangent line in dual coordinates).  When composed
symmetrically (half primal, full dual, half primal via the inversion),
the first-order errors cancel, giving second-order accuracy.

This is the same principle as the St\"ormer--Verlet method in mechanics:
a half-step in position, a full step in momentum, a half-step in
position.  The Legendre transform $\lambda = -\nabla F(s)$ plays the
role of the Legendre transform $p = \partial L / \partial \dot{q}$
that relates position and momentum.
