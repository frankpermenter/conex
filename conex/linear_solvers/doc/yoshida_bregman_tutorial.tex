...
\subsection{Tangent line (0th-order Euler)}
...
For a self-concordant barrier,
\[
  \|\gamma(h) - (s_0 + hv)\|_H \leq \frac{\delta^2}{2} + O(\delta^3),
  \qquad \delta = h\|v\|_H.
\]
\end{theorem}
\begin{proof}
The leading error is $\frac{h^2}{4}\|H^{-1}T(v,v)\|_H
= \frac{h^2}{4}\|T(v,v)\|_{H^{-1}}$.
For any $w$ with $\|w\|_H = 1$:
\[
  |w^\top T(v,v)| = |D^3 F[w, v, v]| \leq 2\|v\|_H^2,
\]
by~\eqref{eq:sc}.  Taking the supremum:
$\|T(v,v)\|_{H^{-1}} \leq 2\|v\|_H^2$.  Therefore
\[
  \|\gamma(h) - L(h)\|_H \leq \frac{h^2}{4} \cdot 2\|v\|_H^2
  = \frac{h^2 \|v\|_H^2}{2} = \frac{\delta^2}{2}. \qedhere
\]
\end{proof}

The tangent line stays in the Dikin ellipsoid ($\|\text{error}\|_H < 1$)
whenever $\delta < \sqrt{2}$.

\subsection{Bregman midpoint (2nd order)}

From the Taylor expansion proof (Section~\ref{sec:bregman-proof}),
the Bregman midpoint matches the geodesic to second order.  The
...
\begin{equation}\label{eq:bregman-bound}
  \|\gamma(h) - s_{\text{Bregman}}\|_H \leq C_2\, \delta^3 + O(\delta^4),
\end{equation}
...
\subsection{Yoshida--Bregman (4th and 6th order)}
...
Bregman midpoint  & $C_2 \delta^3$     & 2 & $\delta < C_2^{-1/3}$ \\
...
\section{Improved dependence on the self-concordance parameter}
\label{sec:m-concordance}

Nesterov~\cite{nesterov2018} parameterizes self-concordance with a
constant $M_f$:
\begin{equation}\label{eq:m-concordance}
  |D^3 F(s)[v, v, v]| \leq 2M_f \|v\|_H^3.
\end{equation}
Standard self-concordant barriers have $M_f = 1$.  Larger $M_f$
means the Hessian varies faster---the barrier is ``less regular.''
The classical IPM theory (using tangent-line steps) gives step-size
and iteration bounds that depend on $M_f$.

We show that higher-order integrators \emph{improve the dependence
on $M_f$}: the Yoshida hierarchy reduces the sensitivity from
$M_f^{-1/2}$ (tangent line) to $M_f^{-1/(2p+1)}$ (order $2p$).
This is a new consequence, not present in Nesterov's analysis.

\subsection{Step-size bounds}

From the error bounds in the previous section, the maximum step
$\delta = h\|v\|_H$ for staying in the Dikin ellipsoid depends on
both $M_f$ and the integrator order.

\paragraph{Tangent line (Nesterov's analysis).}
Error $\leq \frac{M_f}{2}\delta^2$.  Interior if
$\delta < \sqrt{2/M_f}$.  This is the classical bound.

\paragraph{Bregman midpoint (new).}
The midpoint quadrature error is controlled by the variation of
$\dot\lambda = -Hv$, which involves $D^3 F$.  The bound:
\[
  \text{error} \leq C_2 \, M_f \, \delta^3,
\]
where $C_2$ is a universal constant.
Interior if $\delta < (C_2 M_f)^{-1/3}$.

\paragraph{Yoshida-$2p$ (new).}
The Yoshida composition cancels error terms up to order $2p$:
\[
  \text{error} \leq C_p \, M_f^{p} \, \delta^{2p+1}.
\]
Interior if $\delta < (C_p \, M_f^p)^{-1/(2p+1)}$.

\subsection{The improvement: integrator order reduces $M_f$ sensitivity}

For fixed accuracy (error $= \varepsilon$), the required step size is:
\[
  \delta_{\max} \propto \left(\frac{\varepsilon}{M_f^p}\right)^{1/(2p+1)}.
\]
As $M_f$ increases (worse barrier), $\delta_{\max}$ shrinks.  But the
rate of shrinkage depends on the method order $2p$:

\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{@{}lccccc@{}}
\toprule
& & \multicolumn{4}{c}{\textbf{Max step $\delta$ for $\|\text{error}\|_H < 1$}} \\
\cmidrule(l){3-6}
\textbf{Method} & \textbf{Order} & $M_f\!=\!1$ & $M_f\!=\!50$ & $M_f\!=\!5000$ & $M_f\!=\!5 \!\times\! 10^5$ \\
\midrule
Tangent line      & 1 & $1.00$ & $0.14$ & $0.014$ & $0.0014$ \\
Bregman midpoint  & 2 & $\sim\!1$   & $0.22$ & $0.046$ & $0.010$ \\
Yoshida-4         & 4 & $\sim\!1$   & $0.40$ & $0.16$  & $0.063$ \\
Yoshida-6         & 6 & $\sim\!1$   & $0.52$ & $0.27$  & $0.14$ \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Reading the table.}
At $M_f = 5 \times 10^5$ (far from self-concordant), the tangent line
requires $\delta < 0.0014$ (thousands of tiny steps), while Yoshida-6
permits $\delta < 0.14$ (a few large steps).  The higher-order method
compensates for the poor regularity of the barrier.

\subsection{What this enables}

\begin{enumerate}
  \item \textbf{Approximate barriers.}  A cheap approximation to $F$
    (polynomial fit, interpolation, neural network) may have $M \gg 2$
    but fast $\nabla F$ and $\nabla^2 F$ evaluations.  Yoshida
    compensates for the large $M$.

  \item \textbf{Universal barriers.}  The universal barrier exists
    for any proper cone but has large barrier parameter $\nu$ and
    poor concordance constants.  Higher-order integrators make it
    practical by tolerating the large $M$.

  \item \textbf{Composite barriers.}  $F = F_1 + \varepsilon F_2$
    where $F_1$ is a good barrier for most of the cone and $F_2$
    handles a difficult boundary region.  The perturbation
    $\varepsilon F_2$ may have large $M$ but small overall effect;
    Yoshida tolerates it.

  \item \textbf{Exotic cones.}  For cones where constructing an
    $M = 2$ barrier is hard (matrix power cones, quantum entropy
    cones), an $M = 100$ barrier may be straightforward.  Classical
    IPMs (which assume $M = 2$) cannot use such barriers; Yoshida-based
    IPMs can.
\end{enumerate}

\paragraph{The new result.}
Nesterov's classical analysis gives step size $\delta \propto M_f^{-1/2}$
for the tangent-line step.  The Yoshida hierarchy improves this to:
\[
  M_f^{-1/2} \;\to\; M_f^{-1/3} \;\to\; M_f^{-1/5} \;\to\; M_f^{-1/7} \;\to\; \cdots
\]
Each level reduces the sensitivity to $M_f$, making barriers with
large self-concordance parameter practical at the cost of a few extra
gradient and Hessian evaluations per step.

For barriers with $M_f = 1$ (standard self-concordant), the improvement
manifests as larger feasible step sizes (effectively reducing the
barrier parameter $\nu$).  For barriers with $M_f \gg 1$, the
improvement is qualitative: the Yoshida-based IPM converges where the
classical tangent-line IPM cannot take meaningful steps.
