...
void T::UpdateData() {
  ...
  if (n1 == source_submatrix.rows()) {
    new (&source_submatrix)
        Eigen::Map<Eigen::MatrixXd, Eigen::Aligned>(kkt_subsystem_->supernode_submatrix().data(), n1, n1);
  ...
  assembler_->SetDenseData();
  AssignSubmatrix(source_submatrix,
                  Q_in_elimination_order_, kkt_subsystem_->variable_to_local_elimination_rank());
  
  kkt_subsystem_->supernode_submatrix().triangularView<Eigen::Lower>() = Q_in_elimination_order_.topLeftCorner(n1, n1).triangularView<Eigen::Lower>();
  kkt_subsystem_->separator_rows() = Q_in_elimination_order_.bottomLeftCorner(n2, n1);
  kkt_subsystem_->separator_schur_complement().triangularView<Eigen::Lower>() = Q_in_elimination_order_.bottomRightCorner(n2, n2).triangularView<Eigen::Lower>();
}
