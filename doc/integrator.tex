...
\begin{algorithm}
\caption{Symmetric Geodesic Step (Log-Homogeneous)}
\label{alg:symmetric_step}
\begin{algorithmic}[1]
\Require{Current position $z_k \in \text{int}(K)$, velocity $v_k \in T_{z_k}K$, step size $h$}
\Ensure{Updated state $(z_{k+1}, v_{k+1})$}

\State $w_k \gets H(z_k)v_k$ \Comment{Compute initial dual momentum}
\State $\text{Target} \gets 2h w_k$ 

\State \textbf{Solve for } $z_{k+1}$: \Comment{Implicit Primal Update}
\State \quad $\nabla \phi(z_{k+1}) + H(z_k)z_{k+1} = \text{Target}$ \label{alg:implicit_solve}

\State $\lambda_{k+1} \gets \nabla \phi(z_{k+1})$
\State $\lambda_k \gets \nabla \phi(z_k)$ \Comment{Note: $\lambda_k = -H(z_k)z_k$ by homogeneity}

\State $w_{k+1} \gets \frac{1}{2h} \left[ (\lambda_{k+1} - \lambda_k) + H(z_{k+1})(z_{k+1} - z_k) \right]$ \label{alg:momentum_update}
\State $v_{k+1} \gets H(z_{k+1})^{-1} w_{k+1}$ \Comment{Map back to tangent space}

\State \Return $(z_{k+1}, v_{k+1})$
\end{algorithmic}
\end{algorithm}
...
\subsection{The dual midpoint method}

We establish that the Primal Midpoint (P-Mid) and Dual Midpoint (D-Mid) integrators are formally symmetric for any strictly convex barrier $\phi$. This property, also known as self-adjointness, is a prerequisite for high-order Yoshida composition.

\begin{definition}[D-Mid Discrete Flow]
The Dual Midpoint flow map $\Psi_h: (z_0, w_0) \mapsto (z_1, w_1)$ is defined by the following system:
\begin{align}
    \lambda_{1/2} &= \nabla \phi(z_0) + \frac{h}{2}w_0 \\
    z_1 &= 2\nabla \phi^*(\lambda_{1/2}) - z_0 \\
    w_1 &= \frac{2}{h}(\nabla \phi(z_1) - \lambda_{1/2})
\end{align}
\end{definition}

\begin{theorem}[Formal Symmetry]
The D-Mid integrator is symmetric: $\Psi_h = \Psi_{-h}^{-1}$ for any convex barrier $\phi$.
\end{theorem}

\begin{proof}
To prove symmetry, we must show that starting from $(z_1, w_1)$ and applying $\Psi_{-h}$ recovers $(z_0, w_0)$. Let $\Psi_{-h}(z_1, w_1) = (z_2, w_2)$.

\paragraph{1. Midpoint Invariance:}
The backward dual midpoint $\tilde{\lambda}_{1/2}$ is given by:
\begin{equation}
    \tilde{\lambda}_{1/2} = \nabla \phi(z_1) + \frac{-h}{2}w_1
\end{equation}
Substituting the definition of $w_1$ from the forward step:
\begin{equation}
    \tilde{\lambda}_{1/2} = \nabla \phi(z_1) - \frac{h}{2} \left[ \frac{2}{h}(\nabla \phi(z_1) - \lambda_{1/2}) \right] = \nabla \phi(z_1) - (\nabla \phi(z_1) - \lambda_{1/2}) = \lambda_{1/2}
\end{equation}
The midpoint in the dual space is invariant under the time-reversal operation.

\paragraph{2. Primal Recovery:}
The backward primal update $z_2$ is:
\begin{equation}
    z_2 = 2\nabla \phi^*(\tilde{\lambda}_{1/2}) - z_1 = 2\nabla \phi^*(\lambda_{1/2}) - z_1
\end{equation}
From the forward step definition, we know $2\nabla \phi^*(\lambda_{1/2}) = z_1 + z_0$. Substituting this:
\begin{equation}
    z_2 = (z_1 + z_0) - z_1 = z_0
\end{equation}

\paragraph{3. Momentum Recovery:}
The backward momentum $w_2$ is:
\begin{equation}
    w_2 = \frac{2}{-h}(\nabla \phi(z_2) - \tilde{\lambda}_{1/2}) = -\frac{2}{h}(\nabla \phi(z_0) - \lambda_{1/2}) = \frac{2}{h}(\lambda_{1/2} - \lambda_0)
\end{equation}
Substituting $\lambda_{1/2} - \lambda_0 = \frac{h}{2}w_0$ from the forward step:
\begin{equation}
    w_2 = \frac{2}{h} \left( \frac{h}{2}w_0 \right) = w_0
\end{equation}

Since $\Psi_{-h}(z_1, w_1) = (z_0, w_0)$, the integrator is symmetric. A parallel argument holds for the Primal Midpoint (P-Mid) integrator.
\end{proof}

\subsection{The primal midpoint method}

The Primal Midpoint integrator is the primal-dual mirror of the D-Mid scheme. It prioritizes linearization in the primal space, making it particularly effective when the primal barrier $\phi$ is computationally simpler to evaluate than its conjugate $\phi^*$.

\begin{definition}[P-Mid Discrete Flow]
The Primal Midpoint flow map $\Psi_h: (z_0, v_0) \mapsto (z_1, v_1)$ is defined by the following sequence of operations:
\begin{align}
    z_{1/2} &= z_0 + \frac{h}{2}v_0 \label{eq:pmid_halfstep} \\
    \lambda_1 &= 2\nabla \phi(z_{1/2}) - \nabla \phi(z_0) \label{eq:pmid_extrap} \\
    z_1 &= \nabla \phi^*(\lambda_1) \label{eq:pmid_map} \\
    v_1 &= \frac{2}{h}(z_1 - z_{1/2}) \label{eq:pmid_vel}
\end{align}
...
Given the state at time $t$, $(z_0, v_0)$, the state at time $t+h$ is defined by:
\begin{subequations}
\label{eq:full_integrator}
\begin{align}
    \nabla \phi(z_1) - \nabla \phi(z_0) &= H(z_0)(2hv_0 - (z_1 - z_0)) \label{eq:pos_update} \\
    \nabla \phi(z_1) - \nabla \phi(z_0) &= H(z_1)(2hv_1 - (z_1 - z_0)) \label{eq:vel_update}
\end{align}
\end{subequations}
\end{definition}
...
We now establish that the discrete flow map $\Psi_h$ is a consistent second-order approximation of the geodesic flow on the Hessian manifold. We achieve this by demonstrating that both the position and velocity Taylor expansions of the map agree with the exact derivatives of the Riemannian geodesic.

\begin{theorem}[Second-Order Flow Agreement]
Let $\gamma(h)$ be the exact Riemannian geodesic starting at $(z_0, v_0)$. The discrete flow map $\Psi_h(z_0, v_0) = (z_1, v_1)$ satisfies:
\begin{equation}
    z_1(h) = \gamma(h) + \mathcal{O}(h^3), \quad v_1(h) = \gamma'(h) + \mathcal{O}(h^3)
\end{equation}
\end{theorem}
...
\begin{proof}
The state $(z_1, v_1)$ is determined by the system \eqref{eq:full_integrator}. We expand both variables as Taylor series in $h$: $z_1(h) = a_0 + h a_1 + \frac{h^2}{2} a_2 + \mathcal{O}(h^3)$ and $v_1(h) = b_0 + h b_1 + \mathcal{O}(h^2)$.

\paragraph{1. Position Accuracy:}
View the $z_1$ satisfying \eqref{eq:pos_update} as a function $z_t(h)$ of $h$.
At $h=0$, \eqref{eq:pos_update} reduces to $\nabla \phi(z_1(0)) - \nabla \phi(z_0) = H(z_0)(0 - (z_1(0) - z_0))$, which implies $a_0 = z_0$.

Differentiating both sides of \eqref{eq:pos_update} with respect to $h$:
\begin{equation}
    H(z_1)z_1' = H(z_0)(2v_0 - z_1')
\end{equation}
Evaluating at $h=0$, where $z_1 = z_0$ and $H(z_1) = H(z_0)$:
\begin{equation}
    H(z_0)z_1'(0) = 2H(z_0)v_0 - H(z_0)z_1'(0) \implies 2H(z_0)z_1'(0) = 2H(z_0)v_0
\end{equation}
This gives $a_1 = z_1'(0) = v_0$, matching the initial geodesic velocity $\gamma'(0)$.

Differentiating \eqref{eq:pos_update} a second time:
\begin{equation}
    \nabla^3 \phi(z_1)[z_1', z_1', \cdot] + H(z_1)z_1'' = -H(z_0)z_1''
\end{equation}
...
\paragraph{2. Velocity Accuracy:}
Since \eqref{eq:vel_update} is an identity of the form $0=0$ at $h=0$, we define the velocity residual function $G(v_1, z_1, h) = 2h H(z_1)v_1 - (\nabla \phi(z_1) - \nabla \phi(z_0)) - H(z_1)(z_1 - z_0) = 0$. Because $G \equiv 0$, all derivatives with respect to $h$ must vanish. The first derivative $\frac{dG}{dh} = 0$ is:
\begin{equation}
    2H(z_1)v_1 + 2h\frac{d}{dh}[H(z_1)v_1] - H(z_1)z_1' - \left( \nabla^3 \phi(z_1)[z_1', z_1 - z_0] + H(z_1)z_1' \right) = 0
\end{equation}
Evaluating at $h=0$, noting that $z_1(0) - z_0 = 0$ and the $2h$ term vanishes:
\begin{equation}
    2H(z_0)v_1(0) - 2H(z_0)z_1'(0) = 0 \implies b_0 = v_0
\end{equation}
To find $b_1 = v_1'(0)$, we differentiate $G$ a second time and evaluate at $h=0$. Grouping terms:
\begin{equation}
    \frac{d^2G}{dh^2} = 4\frac{d}{dh}[H(z_1)v_1] - 2\frac{d}{dh}[H(z_1)z_1'] - \frac{d}{dh}\left( \nabla^3 \phi(z_1)[z_1', z_1 - z_0] \right) = 0
\end{equation}
Applying the chain rule and evaluating at $h=0$ (where $z_1' = v_1 = v_0$ and $z_1 - z_0 = 0$):
\begin{itemize}
    \item $\left( 4\frac{d}{dh}[H(z_1)v_1] \right)_{|h=0} = 4(\nabla^3 \phi(z_0)[v_0, v_0, \cdot] + H_0 b_1)$
    \item $\left( 2\frac{d}{dh}[H(z_1)z_1'] \right)_{|h=0} = 2(\nabla^3 \phi(z_0)[v_0, v_0, \cdot] + H_0 a_2)$
    \item $\left( \frac{d}{dh} \nabla^3 \phi(z_1)[z_1', z_1 - z_0] \right)_{|h=0} = \nabla^3 \phi(z_0)[v_0, v_0, \cdot]$
\end{itemize}
...
\begin{equation}
    4H_0 b_1 + 4\nabla^3 \phi[v_0, v_0] - 2\nabla^3 \phi[v_0, v_0] - 2H_0 a_2 - \nabla^3 \phi[v_0, v_0] = 0
\end{equation}
Substituting $H_0 a_2 = -\frac{1}{2}\nabla^3 \phi[v_0, v_0]$ from the position analysis:
\begin{equation}
...
Since the Taylor coefficients of both position $z_1$ and velocity $v_1$ match the continuous geodesic derivatives through the second order, the discrete flow map $\Psi_h$ is a consistent second-order integrator.
\end{proof}
