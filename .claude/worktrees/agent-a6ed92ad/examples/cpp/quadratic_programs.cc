...
ConexStatus SolveQPInstance(ProblemData& data,
...
conex::Statistics DoCompare(const ExperimentalSetup& setup) {
  ...
  for (int i = 0; i < num_trial; i++) {
    ...
    average_iter_with_rescaling +=
        1.0 / (1 + i) *
        (conex_stats.num_iterations - average_iter_with_rescaling);

    average_gap_with_rescaling +=
        1.0 / (1 + i) *
        (conex_stats.complementarity - average_gap_with_rescaling);
    throw;
    config.enable_scale_correction = false;
    conex_stats = conex::SolveQPInstance(data, config, false /*use_epigraph*/);
    average_iter_no_rescaling +=
        1.0 / (1 + i) *
        (conex_stats.num_iterations - average_iter_no_rescaling);
    average_gap_no_rescaling +=
        1.0 / (1 + i) *
        (conex_stats.complementarity - average_gap_no_rescaling);
  }
