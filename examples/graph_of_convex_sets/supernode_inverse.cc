...
class DenseBlock : public DenseBlockBase {
 ...
 public:
  static int SupernodeSize(const Parameters& p) {
    return (p.spatial_dimension + 1) * (p.num_edges + 1);
  }
  ...
  DenseBlock(const Parameters& p, Eigen::Ref<MatrixXd> full_matrix)
      : DenseBlockBase(
            full_matrix.bottomRightCorner(SupernodeSize(p), SupernodeSize(p)),
            dummy, dummy) {
  ...
  void SetData(Eigen::Ref<MatrixXd> full_matrix) {
    supernode_submatrix() =
        full_matrix.bottomRightCorner(supernodes().size(), supernodes().size());
  }
};
...
class IncomingSpatialVariableBlock : public IncomingSpatialVariableBlockBase {
 ...
 public:
  ...
  static int EdgeSeparatorGlobalOffset(const Parameters& p, int edge_number) {
    return p.spatial_dimension * p.num_edges +
           edge_number * (p.spatial_dimension + 1);
  }
  ...
  IncomingSpatialVariableBlock(const Parameters& p, int edge_number) {
    ...
    std::iota(supernodes.begin(), supernodes.end(),
              SupernodeGlobalOffset(p, edge_number));
    SetSupernodes(supernodes);
    ...
    std::iota(separators.begin(), separators.begin() + seperator_global_size_1_,
              seperator_global_offset_1_);

    seperator_global_offset_2_ = p.num_edges * (2 * p.spatial_dimension + 1);
    seperator_global_size_2_ = p.spatial_dimension;
    std::iota(separators.begin() + seperator_global_size_1_, separators.end(),
              seperator_global_offset_2_);
  ...
  void SetData(Eigen::Ref<MatrixXd> full_matrix) {
    ...
    supernode_submatrix() =
        full_matrix.block(offset, offset, size_super, size_super);
    separator_rows().topRows(seperator_global_size_1_) =
        full_matrix.block(seperator_global_offset_1_, offset,
                          seperator_global_size_1_, size_super);
    separator_rows().bottomRows(seperator_global_size_2_) =
        full_matrix.block(seperator_global_offset_2_, offset,
                          seperator_global_size_2_, size_super);
...
T::SupernodeSubmatrix(const Parameters& p,
                      Eigen::Ref<Eigen::MatrixXd> full_matrix)
  ...
  for (int i = 0; i < p.num_edges; i++) {
    incoming_blocks_.at(i) =
        std::make_unique<IncomingSpatialVariableBlock>(p, i);
  }
