...
SupernodeSubmatrix::Parameters MakeParams(
    const ConvexSetNodeParameters& input) {
...
void T::DoInitialize() {
  ...
  supernode_submatrix_ = std::make_unique<SupernodeSubmatrix>(
      MakeParams(params_), supernode_submatrix());
}
...
Eigen::MatrixXd T::MakeSuperNodeSubmatrix() {
  ...
  for (int i = 0; i < num_incoming; i++) {
    for (int j = 0; j < num_incoming; j++) {
      ...
      Q.block(params_.incoming_spatial_flow_start_positions.at(i),
              params_.incoming_spatial_flow_start_positions.at(j), spatial_dim,
              spatial_dim) = edge_hessian;
      ...
      Q.block(
           params_.outgoing_spatial_flow_of_incoming_edge_start_positions.at(i),
           params_.incoming_flow_start_positions.at(j), spatial_dim, 1)
          .setConstant(0.01);
      ...
      Q.block(params_.incoming_flow_start_positions.at(i),
              params_.incoming_spatial_flow_start_positions.at(j), 1,
              spatial_dim)
          .setConstant(0.01);
    }
  }
  ...
  for (int i = 0; i < num_incoming; i++) {
    Q.block(params_.conservation_of_spatial_flow_multiplier_position,
            params_.incoming_spatial_flow_start_positions.at(i), spatial_dim,
            spatial_dim)
        .setConstant(.001);
    Q.block(params_.conservation_of_spatial_flow_multiplier_position,
            params_.incoming_spatial_flow_start_positions.at(i), spatial_dim,
...
Eigen::MatrixXd T::MakeSeperatorMatrix() {
    ...
    for (int i = 0; i < num_outgoing; i++) {
      for (int j = 0; j < num_incoming; j++) {
        Q.block(params_.outgoing_flow_start_positions.at(i),
                params_.incoming_flow_start_positions.at(j), spatial_dim,
                spatial_dim)
            .setConstant(.000001);
      }
    }
  ...
  for (int i = 0; i < num_outgoing; i++) {
    Q.block(params_.outgoing_spatial_flow_start_positions.at(i),
            params_.conservation_of_spatial_flow_multiplier_position,
            spatial_dim, spatial_dim)
        .setConstant(.01);
    Q.block(params_.outgoing_spatial_flow_start_positions.at(i),
            params_.conservation_of_spatial_flow_multiplier_position,
...
int T::LookUpPosition(int edge_position, VariablePartition row_block) {
  ...
  switch (row_block) {
    ...
    case VariablePartition::outgoing_spatial_variable: {
      row = params_.outgoing_spatial_flow_of_incoming_edge_start_positions.at(
          edge_position);
      break;
    }
...
Eigen::Ref<Eigen::MatrixXd> T::quadratic_cost_mutable(
    int edge_id, VariablePartition row_block, VariablePartition col_block) {
  const Node& node_info = params_.graph->node(params_.global_node_label);
  int edge_position =
      std::distance(node_info.incoming_edges.begin(),
                    std::find(node_info.incoming_edges.begin(),
                              node_info.incoming_edges.end(), edge_id));
  CONEX_CHECK(static_cast<int>(edge_position) <
              node_info.incoming_edges.size());
  ...
  return factorization_->supernode_submatrix().block(row, col, size_row,
                                                     size_col);
}
