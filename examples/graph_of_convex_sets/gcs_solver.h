...
/* Given a directed, acyclic graph (DAG) with vertex set V, edgeset E, source
 * vertex s and sink vertex t, this class solves an optimization problems of the
 * form
 *
 *    min.       \sum_{e \in E} f_e (w_e)
 *    subject to. flow constraints,
 *
 * where f_e is a quadratic polynomial in edge variable w_e. The flow
 * constraints are defined by partitioning each w_e as
 *
 *     w_e = (y_e, z_e, phi_e) \in R^{d_u} \times R^{d_v} \times R,
 *
 * where d_u and d_v denote the "spatial dimensions" of the vertices u \in V and
 * v \in V connected by e \in E.  They take the form:
 *
...
class GraphSolver {
 public:
  /*  Throws if the graph contains cycles, has multiple sources or has multiple
   * sinks*/
  GraphSolver(const GraphData& graph_data,
              const std::vector<int>& topological_order_position_to_node = {});

  // Provides direct access to the quadratic cost matrices for each block of
  // f_e.
  Eigen::Ref<Eigen::MatrixXd> quadratic_edge_cost_mutable(
      int edge_number, VariablePartition row_block,
      VariablePartition col_block);
  Eigen::Ref<Eigen::MatrixXd> linear_cost(int edge_number,
                                          VariablePartition row_block);
